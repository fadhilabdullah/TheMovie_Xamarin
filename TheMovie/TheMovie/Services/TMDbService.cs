...
namespace TheMovie.Services
{
    ...
    {
        private const string apiKey = "e377867072accc7428b9ba5d2fe1847f";
        private const string baseUrl = "https://api.themoviedb.org/3";

        private const string searchMoviePath = "/search/movie";
        private const string moviePath = "/movie";
        private const string genreListPath = "/genre/movie/list";
        ...
        public async Task<SearchMovie> GetFilterGenresAsync(int id, int page)
        {
            var restUrl = $"{baseUrl}?api_key={apiKey}&with_genres={id}";
            try
            {
                using (var response = await httpClient.GetAsync(restUrl).ConfigureAwait(false))
                {
